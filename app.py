...
def process_segment_for_table(job, segment, s3_client):
    try:
        # Load segment data and calculate metrics
...
def main():
    ...
    # --- Insert Combined CSS & JS Styling (from your second code) ---
    st.markdown("""
    <style>
      ...
      .sidebar-box h2, [data-testid="stSidebar"] .stSelectbox > label {
      ...
      .shotmetrics-title {
          ...
          font-weight: 400;
          color: #FFFFFF !important;
          text-shadow: 0 0 5px #FFFFFF, 0 0 10px #2E3E4F, 2px 2px 4px rgba(0,0,0,0.4);
          margin: 0;
    ...
    # Example filters (by season, team, player, date, source, shot type)
    ...
    # --- Show Aggregated Stats or Detailed View if a row is selected ---
    if len(selected_rows) > 1:
        st.subheader("Aggregate Statistics")
        agg_df = selected_rows[["Distance (ft)", "Release Angle", "Release Velocity", 
                                 "Apex Height", "Release Time", "Side Curvature", 
                                 "Rear Curvature", "Lateral Deviation"]].agg(['min', 'max', 'mean', 'std'])
    ...
    if not selected_rows.empty:
        ...
        metrics = {
            ...
            'release_height': selected_row['Release Height'],
...
def show_biomechanics_page(df_pose, df_ball, df_spin, metrics):
    ...
    with col1:
        animated_flip_kpi_card(
            ...
            calculation_info="Sum of timing, range, and release angle scores."
        )
    with col2:
        animated_flip_kpi_card(
            "Stability Ratio",
    ...
    with col5:
        animated_flip_kpi_card(
            ...
            calculation_info="Average difference between left and right knee/elbow angles."
        )
    with col6:
        animated_flip_kpi_card(
            "Shoulder Rotation",
            ...
            calculation_info="Difference in shoulder angle from lift to release."
        )
    with col7:
        animated_flip_kpi_card(
            "COM Acceleration",
            ...
            description="Positive values indicate upward/forward motion.",
            calculation_info="Mean acceleration of the centroid."
        )
